...
if __name__ == "__main__":
    ...
    s = os.path.join(PLOTS_PATH, "grad_norm.pth")
    print(f"save to {s}")
    torch.save([selected_epochs, grad_norm_list], s)

    phases_k = ['pre_memo_epoch', 'pre_comp_epoch', 'memo_epoch', 'comp_epoch']
    phases = {k : states[k] for k in phases_k}

    save_file_name = os.path.join(PLOTS_PATH, f"grad_norm_{train_data_pct}")

    plot_grad_norm(
        selected_epochs, grad_norm_list, save_file_name=save_file_name, phases=phases, ax=None,
